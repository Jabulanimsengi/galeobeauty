...
export function ServicesClient({ categoryParam, queryParam = "" }: ServicesClientProps) {
    ...
    return (
        <>
            ...
            <main className="bg-background min-h-screen">
                {/* Hero */}
                <section className="relative pt-32 pb-8 lg:pt-40 lg:pb-10 px-6 bg-stone-50/40">
                    <div className="container mx-auto text-center max-w-4xl">
                        <span className="text-gold font-medium uppercase tracking-widest text-xs sm:text-sm block mb-4">
                            Galeo Beauty Hartbeespoort Treatment Menu
                        </span>
                        <h1 className="font-sans text-4xl sm:text-5xl lg:text-6xl text-foreground mb-6">
                            Beauty <span className="text-gold">Services</span> in Hartbeespoort
                        </h1>
                                                                    ...
                                                                    <div key={subcategory.id} className="space-y-2">
                                                                        {subcategory.items.map((item) => (
                                                                            <TreatmentListItem
                                                                                ...
                                                                                detailsLink={`/services/${category.id}/${item.id}`}
                                                                            />
                                                                        ))}
                                                                    </div>
                                                                ))}
            ...
            {/* Mobile Sticky Cart (only visible on mobile/tablet after a treatment is selected) */}
            {selectedTreatments.length > 0 && (
