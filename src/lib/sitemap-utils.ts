...
export function calculateSitemapURLCounts() {
    ...
    const guidePages = getPublishedGuidePages();
    ...
    const sitemap0Total = STATIC_PAGES + guidePages.length + blogPosts.length + bespokeServicePages.length + sitemap0ServicePages + 1 + sitemap0Locations + sitemap0LocationServices;
...
export function printSitemapCounts() {
    ...
    console.log(`Guide pages:               ${counts.sitemap0.guidePages.toLocaleString()}`);
