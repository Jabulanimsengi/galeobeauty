...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // === Add Booking Buttons to Price Rows ===
    // Auto-add booking buttons to all price rows on prices page
    const priceRows = document.querySelectorAll('.pricing-section .price-row');
    console.log('Found price rows:', priceRows.length);

    priceRows.forEach(row => {
        // Skip if already has booking button
        if (row.querySelector('.price-row-book')) return;

        // Add the has-booking class
        row.classList.add('has-booking');

        // Create booking link
        const bookLink = document.createElement('a');
        bookLink.href = 'contact.html#booking';
        bookLink.className = 'price-row-book';
